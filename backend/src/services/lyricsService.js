...
async function scrapeLyricsFromGenius(url, title, artist) {
  ...
  try {
    ...
    const rawLyrics = await page.$$eval("[data-lyrics-container]", (containers) =>
    ...
    console.log(`✅ Successfully cleaned and formatted lyrics!`);

    if (!lyrics) {
      console.warn(`⚠️ No lyrics found on page, please check: ${url}`);
      await browser.close();
      return null;
    }
