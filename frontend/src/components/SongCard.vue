  ...
  <div class="song-card w-full flex flex-col p-4 shadow-lg border-t transition-colors" :data-id="song.id" :data-song-id="song.melon_song_id">
    <div class="flex items-center gap-4 w-full">
      ...
      <div class="flex items-center min-w-[50px] justify-center gap-1">
        ...
        <span
          class="flex items-end text-sm gap-0.5"
          :class="{
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          @click="toggleExpand"
          class="px-3 py-1 text-sm font-medium rounded-lg cursor-pointer transition-colors bg-[var(--p-primary-color)] text-white hover:bg-[var(--p-primary-400)]"
        >
          Lyrics
        </button>
        <div class="relative">
          ...
          <button
            ...
            class="text-slg font-medium rounded-lg cursor-pointer transition-colors text-[var(--p-primary-color)] hover:text-[var(--p-primary-400)]"
          >
            +
          </button>
          <!-- ✅ Playlist Dropdown (absolutely positioned so it won’t stretch SongCard) -->
          <div
            v-if="showPlaylistMenu"
            class="absolute right-0 mt-2 w-48 shadow-lg rounded-lg z-50"
          >
            <!-- Inner wrapper: separates the scrollable area from the fixed overlay -->
            <div class="relative">
              <!-- ✅ Scrollable Dropdown Content -->
              <div
                ref="playlistMenuRef"
                class="overflow-y-auto max-h-64 scrollbar-hidden rounded-t-lg rounded-b-lg" 
                :class="{
              ...
              >
                <button
                  @click="toggleNewPlaylistInput"
                  class="w-full text-center px-3 py-2 text-xs font-medium hover:bg-gray-100 rounded-t-lg"
                  :class="{
                ...
                <div
                  class="w-48 shadow-lg z-50 rounded-b-lg"
                  :class="{
                    'bg-[var(--p-surface-50)] text-black': !isDarkMode,
                    'bg-[var(--p-surface-100)] text-white': isDarkMode
                  }"
              ...
              <!-- ✅ Overlayed Scroll Indicator (fixed relative to the dropdown wrapper) -->
              <ScrollIndicator 
                class="absolute bottom-0 left-0 w-full pointer-events-none"
                  v-if="!isLoading" :showScrollIndicator="showScrollIndicator"
              />
            </div>
          </div>
        </div>
